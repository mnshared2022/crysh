...
*Setup instructions for a gitian build of crysh Coin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the cryshcoin
...
to the cryshcoin GitHub release page.
...
- [Building crysh Coin](#building-cryshcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/cryshcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/cryshcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/cryshcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/cryshcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/cryshcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/cryshcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/cryshcoin/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@crysh.online) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/cryshcoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/cryshcoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/cryshcoin-osx-*-build.assert
```
