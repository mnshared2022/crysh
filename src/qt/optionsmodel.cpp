...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::crysh);
    ...
    if (settings.contains("nAnonymizecryshcoinAmount") || reset)
        SoftSetArg("-anonymizecryshcoinamount", settings.value("nAnonymizecryshcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in cryshcoin.cpp during shotdown to also remove the window positions
