...
#include "ui_zcryshcontroldialog.h"
...
std::set<std::string> ZcryshControlDialog::setSelectedMints;
std::set<CMintMeta> ZcryshControlDialog::setMints;

bool CZcryshControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZcryshControlDialog::COLUMN_DENOMINATION || column == ZcryshControlDialog::COLUMN_VERSION || column == ZcryshControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZcryshControlDialog::ZcryshControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZcryshControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zcrysh Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZcrysh->setProperty("cssClass", "text-main-purple");
    ui->labelZcrysh_int->setProperty("cssClass", "text-main-purple");
...
ZcryshControlDialog::~ZcryshControlDialog()
{
    delete ui;
}

void ZcryshControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZcryshControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZcryshControlWidgetItem* itemDenom(new CZcryshControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZcryshControlWidgetItem *itemMint = new CZcryshControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zcryshTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zcryshTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zcrysh.";
            else if (!mint.isSeedCorrect)
                strReason = "The zcrysh seed used to mint this zcrysh is not the same as currently hold in the wallet";
            else
...
void ZcryshControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZcryshControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZcrysh_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZcryshControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZcryshControlDialog::GetSelectedMints()
{
...
void ZcryshControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
