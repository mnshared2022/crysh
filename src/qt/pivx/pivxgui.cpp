...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("CRYSH client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("CRYSH"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("CRYSH"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
