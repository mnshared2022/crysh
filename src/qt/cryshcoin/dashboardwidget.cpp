...
#include "qt/cryshcoin/dashboardwidget.h"
#include "qt/cryshcoin/forms/ui_dashboardwidget.h"
#include "qt/cryshcoin/sendconfirmdialog.h"
#include "qt/cryshcoin/txrow.h"
...
DashboardWidget::DashboardWidget(cryshcoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of crysh staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquarecrysh, "square-chart-crysh");
    setCssProperty(ui->labelSquarezcrysh, "square-chart-zcrysh");
    setCssProperty(ui->labelcrysh, "text-chart-crysh");
    setCssProperty(ui->labelZcrysh, "text-chart-zcrysh");
    ui->labelZcrysh->setVisible(false);
    ui->labelSquarezcrysh->setVisible(false);
    ...
    ui->labelAmountZcrysh->setText("0 zcrysh");
    ui->labelAmountZcrysh->setVisible(false);
    ui->labelAmountcrysh->setText("0 crysh");
    setCssProperty(ui->labelAmountcrysh, "text-stake-crysh-disable");
    setCssProperty(ui->labelAmountZcrysh, "text-stake-zcrysh-disable");
    ...
    ui->btnHowTo->setText(tr("How to get crysh"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("crysh")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool iscrysh = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZcrysh;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair crysh, zcrysh
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            crysh = (pair.first != 0) ? pair.first / 100000000 : 0;
            zcrysh = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalcrysh += pair.first;
            chartData->totalZcrysh += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuescrysh.append(crysh);
        chartData->valueszcrysh.append(zcrysh);

        int max = std::max(crysh, zcrysh);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("crysh");
    set1 = new QBarSet("zcrysh");
    set0->setColor(QColor(112, 112, 112));
    ...
    set0->append(chartData->valuescrysh);
    set1->append(chartData->valueszcrysh);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalcrysh > 0 || chartData->totalZcrysh > 0) {
        setCssProperty(ui->labelAmountcrysh, "text-stake-crysh");
        setCssProperty(ui->labelAmountZcrysh, "text-stake-zcrysh");
    } else {
        setCssProperty(ui->labelAmountcrysh, "text-stake-crysh-disable");
        setCssProperty(ui->labelAmountZcrysh, "text-stake-zcrysh-disable");
    }
    forceUpdateStyle({ui->labelAmountcrysh, ui->labelAmountZcrysh});
    ui->labelAmountcrysh->setText(GUIUtil::formatBalance(chartData->totalcrysh, nDisplayUnit));
    ui->labelAmountZcrysh->setText(GUIUtil::formatBalance(chartData->totalZcrysh, nDisplayUnit, true));
