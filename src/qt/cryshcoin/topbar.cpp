...
#include "qt/cryshcoin/topbar.h"
#include "qt/cryshcoin/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/cryshcoin/lockunlock.h"
#include "qt/cryshcoin/qtutils.h"
#include "qt/cryshcoin/receivedialog.h"
...
TopBar::TopBar(cryshcoinGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopcrysh, ui->labelAmountTopzcrysh}, "amount-small-topbar");
    setCssProperty({ui->labelAmountcrysh, ui->labelAmountzcrysh}, "amount-topbar");
    setCssProperty({ui->labelPendingcrysh, ui->labelPendingzcrysh, ui->labelImmaturecrysh, ui->labelImmaturezcrysh}, "amount-small-topbar");

    ui->labelTitle2->setVisible(false);
    ui->labelAmountTopzcrysh->setVisible(false);
    ui->labelAmountzcrysh->setVisible(false);
    ui->labelTitle5->setVisible(false);
    ui->labelPendingzcrysh->setVisible(false);
    ui->labelTitle6->setVisible(false);
    ui->labelImmaturezcrysh->setVisible(false);
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("cryshcoin")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    CAmount cryshAvailableBalance = balance + delegatedBalance;

    // zcrysh Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalcrysh = GUIUtil::formatBalance(cryshAvailableBalance, nDisplayUnit);
    QString totalzcrysh = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopcrysh->setText(totalcrysh);
    ui->labelAmountTopzcrysh->setText(totalzcrysh);

    // Expanded
    ui->labelAmountcrysh->setText(totalcrysh);
    ui->labelAmountzcrysh->setText(totalzcrysh);

    ui->labelPendingcrysh->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzcrysh->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmaturecrysh->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezcrysh->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
