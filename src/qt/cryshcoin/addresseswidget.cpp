...
#include "qt/cryshcoin/forms/ui_addresseswidget.h"
#include "qt/cryshcoin/addresslabelrow.h"
#include "qt/cryshcoin/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(cryshcoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a cryshcoin address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress cryshAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(cryshAdd.Get(), label.toUtf8().constData(),
                cryshAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
