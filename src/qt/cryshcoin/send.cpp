...
#include "qt/cryshcoin/send.h"
#include "qt/cryshcoin/forms/ui_send.h"
#include "qt/cryshcoin/addnewcontactdialog.h"
#include "qt/cryshcoin/qtutils.h"
#include "qt/cryshcoin/sendchangeaddressdialog.h"
#include "qt/cryshcoin/optionbutton.h"
#include "qt/cryshcoin/sendconfirmdialog.h"
#include "qt/cryshcoin/myaddressrow.h"
#include "qt/cryshcoin/guitransactionsutils.h"
...
#include "zcryshcontroldialog.h"

SendWidget::SendWidget(cryshcoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("crysh");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zcrysh");
    setCssProperty(ui->pushRight, "btn-check-right");
    ui->pushRight->setVisible(false);
    ui->pushLeft->setVisible(false);

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (crysh)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a cryshcoin address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 crysh");
    ...
    setCssProperty(coinIcon, "coin-icon-crysh");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){oncryshSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){oncryshSelected(false);});
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send crysh");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zcrysh");
...
void SendWidget::refreshAmounts() {
    ...
    bool isZcrysh = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZcrysh));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZcrysh ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendcrysh = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendcrysh ? AskPassphraseDialog::Context::Send_crysh : AskPassphraseDialog::Context::Send_zcrysh, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendcrysh) ? send(recipients) : sendZcrysh(recipients)) {
...
bool SendWidget::sendZcrysh(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zcrysh is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zcrysh selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZcryshControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZcryshControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZcrysh(
        ...
        inform(tr("zcrysh transaction sent!"));
        ZcryshControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == Zcrysh_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zcrysh require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zcrysh transaction
        ...
        emit message("zcrysh transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(iscrysh){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any crysh to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZcryshControlDialog *zcryshControl = new ZcryshControlDialog(this);
            zcryshControl->setModel(walletModel);
            zcryshControl->exec();
            ui->btnCoinControl->setActive(!ZcryshControlDialog::setSelectedMints.empty());
            zcryshControl->deleteLater();
        } else {
            inform(tr("You don't have any zcrysh in your balance to select."));
        }
    }
}
...
void SendWidget::oncryshSelected(bool _iscrysh){
    iscrysh = _iscrysh;
    setCssProperty(coinIcon, _iscrysh ? "coin-icon-crysh" : "coin-icon-zcrysh");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress cryshAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(cryshAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(cryshAdd.Get(), dialog->getLabel().toStdString(), "send")) {
