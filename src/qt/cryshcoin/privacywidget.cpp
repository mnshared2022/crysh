...
#include "qt/cryshcoin/privacywidget.h"
#include "qt/cryshcoin/forms/ui_privacywidget.h"
#include "qt/cryshcoin/qtutils.h"
#include "guiutil.h"
#include "qt/cryshcoin/denomgenerationdialog.h"
#include "qt/cryshcoin/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(cryshcoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zcrysh anonymizes your crysh by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zcrysh or convert back to crysh"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 crysh ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zcrysh");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zcrysh");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zcrysh");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zcrysh");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zcrysh");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zcrysh");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zcrysh");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zcrysh");
    ...
    // Only Convert to crysh enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzcrysh->setTitleClassAndText("btn-title-grey", "Total 0 zcrysh");
    ui->btnTotalzcrysh->setSubTitleClassAndText("text-subtitle", "Show denominations of zcrysh owned.");
    ui->btnTotalzcrysh->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select crysh outputs to mint into zcrysh.");
    ...
    connect(ui->btnTotalzcrysh, SIGNAL(clicked()), this, SLOT(onTotalZcryshClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zcrysh");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of crysh to mint into zcrysh"));
    }else{
        btnText = tr("Convert back to crysh");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zcrysh to convert back into crysh"));
    }
...
void PrivacyWidget::onTotalZcryshClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzcrysh->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzcrysh->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zcrysh is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zcrysh, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zcrysh").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zcrysh minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZcrysh(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zcrysh converted back to crysh"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zcrysh </b>";
    ...
    ui->btnTotalzcrysh->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
