...
BOOST_AUTO_TEST_CASE(zerocoin_wrapped_serial_spend_test)
{
    ...
    CzcryshWallet *czcryshWallet = new CzcryshWallet(wallet.strWalletFile);
    ...
    for (unsigned int i = 0; i < TESTS_COINS_TO_ACCUMULATE; i++) {
        ...
        czcryshWallet->GenerateDeterministicZcrysh(denom, coin, dMint, true);
        czcryshWallet->UpdateCount();
