// Copyright (c) 2018-2019 The cryshcoin developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zcrysh/deterministicmint.h>
#include "zcryshtracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzcryshTracker::CzcryshTracker(std::string strWalletFile)
{
...
CzcryshTracker::~CzcryshTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzcryshTracker::Init()
{
...
bool CzcryshTracker::Archive(CMintMeta& meta)
{
...
bool CzcryshTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzcryshTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzcryshTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzcryshTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzcryshTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzcryshTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzcryshTracker::GetSerialHashes()
{
...
CAmount CzcryshTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzcryshTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzcryshTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzcryshTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzcryshTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzcryshTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzcryshTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzcryshTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzcryshTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzcryshTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzcryshTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzcryshWallet* zcryshWallet)
{
    bool iszcryshWalletInitialized = (NULL != zcryshWallet);
    CMintMeta meta;
    ...
    if (! iszcryshWalletInitialized)
        zcryshWallet = new CzcryshWallet(strWalletFile);
    meta.isSeedCorrect = zcryshWallet->CheckSeed(dMint);
    if (! iszcryshWalletInitialized)
        delete zcryshWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzcryshTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzcryshTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzcryshTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzcryshTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzcryshTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzcryshTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzcryshWallet* zcryshWallet = new CzcryshWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zcryshWallet);
        }
        delete zcryshWallet;
        LogPrint("zero", "%s: added %d dzcrysh from DB\n", __func__, listDeterministicDB.size());
    }
