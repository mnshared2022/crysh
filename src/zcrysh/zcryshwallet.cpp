...
CzcryshWallet::CzcryshWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zcrysh seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZcryshSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZcryshSeed_deprecated()) {
                    LogPrintf("%s: Updated zcrysh seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcrysh seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcrysh wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzcryshWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZcryshCount(nCountLastUsed);
    else if (!walletdb.ReadZcryshCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzcryshWallet::Lock()
{
    seedMaster = 0;
}

void CzcryshWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzcryshWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZcrysh(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzcryshWallet::LoadMintPoolFromDB()
{
...
void CzcryshWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzcryshWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzcryshWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcryshTracker->HasPubcoinHash(pMint.first)) {
...
bool CzcryshWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZcrysh(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcryshTracker which also adds to database
    pwalletMain->zcryshTracker->Add(dMint, true);
...
void CzcryshWallet::SeedToZcrysh(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzcryshWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzcryshWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZcryshCount(nCountLastUsed);
...
void CzcryshWallet::GenerateDeterministicZcrysh(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzcryshWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzcryshWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzcryshWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
