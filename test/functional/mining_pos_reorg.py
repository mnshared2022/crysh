...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected CRYSH and zCRYSH supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zCRYSHsupply']:
                assert_equal(node['zCRYSHsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check CRYSH and zCRYSH supply at the beginning
        # ------------------------------------------
        # zCRYSH supply: 2 coins for each denomination
        ...
        # CRYSH supply: block rewards minus burned fees for minting
        ...
        # Verify that CRYSH and zCRYSH supplies were properly updated after the spends and reorgs
        self.log.info("Check CRYSH and zCRYSH supply...")
